...
/**
 * Tenant creation service for public use with email authorization. Algorithm of
 * new tenant creation with email confirmation: <li>send a message for specified
 * email with URL including the "secret" token to activate your tenant; <li>if
 * e-mail is good user receive the message and enter the page where you activate
 * tenant and only after that it is created.
 */
...
public class TenantCreator {
  ...
  public TenantCreator(EmailValidationStorage emailValidationStorage,
   ...
   * </table>
   * 
   * @param uuid - tenant identifier in validation queue
  ...
  String uuid) throws CloudAdminException {
    ...
    if (!emailValidationStorage.isValid(uuid)) {
      ...
      return Response.status(Status.BAD_REQUEST)
                     .entity("Your confirmation key is wrong or has already been activated.")
    ...
    if (templateId == null) {
      LOG.warn("TemplateId not found in validation data of tenant {}. Current TemplateId will be used",
               tenantName);
    ...
    // save tenant state
    tenantStateDataManager.waitCreation(tenant(tenantName).templateId(templateId)
                                                          .adminMail(userMail)
                                                          .info());
   ...
   * <tr>
   * <td>400</td>
   * <td>tenant name is not correctly. It can't be null or ''; should contain 20
   * or less characters; should contain lower cased Latin characters (a-z) and
   * digits (0-9); cannot be solely of digits; can't be in black list</td>
   * </tr>
   * <tr>
   * <td>400</td>
   ...
   * </table>
   * 
   * @param tenantName - name for new tenant
   * @param userMail - email of tenant owner
   * @return correspondent status 200
   * @throws CloudAdminException
   * @POST
   * @Path("/create-with-confirm/{tenantname /{user-mail}") //Hide from direct
   *                                         access
   */
  public Response createTenantWithEmailConfirmation(@PathParam("tenantname")
  String tenantName, @PathParam("user-mail")
