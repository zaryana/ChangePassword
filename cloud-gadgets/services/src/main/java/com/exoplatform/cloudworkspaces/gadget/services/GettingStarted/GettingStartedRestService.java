...
public class GettingStartedRestService implements ResourceContainer {
	...
	/**
	 * Get user's getting started status
	 * 
	 * REST service URL: /getting-started/get
	 * 
	...
	public Response set(@PathParam("property") String property, @PathParam("value") Boolean value, @Context SecurityContext sc, @Context UriInfo uriInfo) throws Exception {
		try {
			String userId = ConversationState.getCurrent().getIdentity().getUserId();
			if(userId == null) {
				return Response.status(HTTPStatus.INTERNAL_ERROR).cacheControl(cacheControl).build();
	...
	private boolean hasGadgets(String userId) {
		try{
			...
			UserPortalConfig portalConfig = userPortalConfigService.getUserPortalConfig(userPortalConfigService.getDefaultPortal(), userId);
			...
			for(UserNode node : nodes){
				DataStorage dataStorageService = (DataStorage) ExoContainerContext.getCurrentContainer().getComponentInstanceOfType(DataStorage.class);                
