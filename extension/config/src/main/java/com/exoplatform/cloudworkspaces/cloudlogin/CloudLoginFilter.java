...
public class CloudLoginFilter implements Filter {
  ...
  private static Log logger = ExoLogger.getLogger(CloudLoginFilter.class);

  // TODO: change to cloud extension
  private static final String EXTENSION_SERVLET_CTX = "/cloud";
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
    ...
    if(! isLoginUri && CloudLoginStatus.INITIATED.equals(getCloudLoginService().getStatus(user))) {
      ...
      ServletContext extensionContext = httpServletRequest.getSession().getServletContext().getContext(EXTENSION_SERVLET_CTX);
